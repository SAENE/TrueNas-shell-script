...
BACKUP_FILEDIR=/home/docker/postgres/backup/
#当前日期
DATE=$(date "+%Y%m%d%H%M")
DEL_DAY=31
...
#    fi
#done
echo ----------$BACKUP_FILEDIR/safeline-ce_$DATE.sql.gz BEGIN----------
docker exec -i postgres pg_dump --host "127.0.0.1" --port "5432" --username "" --no-password --format=c --blobs --verbose "safeline-ce" | gzip > ${BACKUP_FILEDIR}/safeline-ce_$DATE.sql.gz
echo ----------$BACKUP_FILEDIR/safeline-ce_$DATE.sql.gz COMPLETE----------
...
echo "done"
chown -R ene:ene ${BACKUP_FILEDIR}
chmod -R 755 ${BACKUP_FILEDIR}